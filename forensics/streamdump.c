...
int main(int argc, char **argv) {
    ...
    long sv_idx = 0;
    const char *sfn = (argc > 3) ? argv[3] : NULL;
    FILE *sout = sfn ? fopen(sfn, "wb") : NULL;
    if (sout) {
        /* header bits: magic(24) version-unary(1) algo(5) mode(3) nplain(32) = 65 */
        unsigned long sacc = 0; int sb = 0;
        for (int b = 0; b < 65; b++) {
            int ev = 0;
            for (int p = 0; p < 3; p++) {
                seed = (seed * 1367208549UL + 1UL) & 0xFFFFFFFFUL;
                long valIdx = sv_idx + (long)(((double)seed / 4294967296.0) * (double)(nsamples - sv_idx));
                ev += evs[valIdx];
                sv_idx++;
            }
            sacc |= (unsigned long)(ev % 2) << sb; sb++;
            if (sb == 8) { fputc(sacc & 0xFF, sout); sacc = 0; sb = 0; }
        }
        unsigned long enc = 128UL + (((unsigned long)157865) + 127UL) / 128UL * 128UL;
        if (strcmp(fn, "image04.jpg") == 0) enc = 128UL + (390393UL + 127UL) / 128UL * 128UL;
        for (unsigned long b = 0; b < enc; b++) {
            int ev = 0;
            for (int p = 0; p < 3; p++) {
                seed = (seed * 1367208549UL + 1UL) & 0xFFFFFFFFUL;
                long valIdx = sv_idx + (long)(((double)seed / 4294967296.0) * (double)(nsamples - sv_idx));
